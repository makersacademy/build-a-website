...
Admittedly, sending a single string 'Hello World' to a browser window is not very useful.  It is, however, worth it to see how much a web framework does for you.  All we had to do in our `app.rb` file was define a **path** (`'/'`) and a response (`'Hello World'`).

The path forms part of the URL to which the browser makes a request - it's the bit that comes after the domain (i.e. `http://localhost:4567/`).  The single slash (`'/'`) denotes the **root path**.
...
Create a new folder in your directory called `views`.  Create a new file inside the `views` folder called `index.erb`, with the following content:
...
Now change `app.rb` to the following:
```ruby
require 'sinatra'
